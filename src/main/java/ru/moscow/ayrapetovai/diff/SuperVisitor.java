 ...
 */
public class SuperVisitor extends PrettyPrintVisitor {
    protected final SuperSourcePrinter printer;
    protected final SuperPrettyPrinterConfiguration configuration;

    public SuperVisitor(SuperPrettyPrinterConfiguration prettyPrinterConfiguration) {
        super(prettyPrinterConfiguration);
        this.configuration = prettyPrinterConfiguration;
        this.printer = new SuperSourcePrinter(configuration.getIndent());
    }
    ...
    public void visit(final BlockStmt n, final Void arg) {
        ...
        if (configuration.isNativePrint() &&
                (!n.getStatements().isEmpty() && n.getStatements().get(0).getBegin().get().line == n.getBegin().get().line
                || n.getStatements().isEmpty() && n.getEnd().get().line == n.getBegin().get().line)) {
            printer.print("{");
        } else {
            printer.println("{");
        }
    ...
    public void visit(final IfStmt n, final Void arg) {
        ...
        if (thenBlock) // block statement should start on the same line
            if (configuration.isNativePrint()) {
                if (n.getThenStmt().getBegin().get().line == n.getThenStmt().getParentNode().get().getBegin().get().line) {
                    printer.print(") ");
                } else
                    printer.println(")");
            } else {
